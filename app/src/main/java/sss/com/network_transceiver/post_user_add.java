...
public class post_user_add extends AsyncTask<String,Integer,Integer>{
	/*動くコードはいいコード*/
	HttpResponse response ;
	final String TAG="post_user_add";
	final Setting_class st= new Setting_class();

	private Callback callback;
	private String requestCode;
	
	private static final String url="http://vps.suzuyoshi-net.com/tomcat/transceiver-server/regist.jsp";
	...
	public post_user_add (final Callback callback, final String requestCode){
	...
	protected Integer doInBackground(String... contents){
		...
		params.add(new BasicNameValuePair("nickname",contents[1]));
		params.add(new BasicNameValuePair("pass1",st.encodePassdigiest(contents[2])));
		params.add(new BasicNameValuePair("pass2",st.encodePassdigiest(contents[3])));
